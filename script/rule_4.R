...
#Rule4: session内に登場しているsml_cdで予約数の多い宿
...
reserved_sml <- yado_merged %>% 
  arrange(sml_cd, desc(count_reserved_mod)) %>% 
  rename(yad_reserved = yad_no, count = count_reserved_mod) %>% 
  ungroup() %>% 
  group_by(sml_cd) %>% 
  mutate(rank = row_number()) %>% 
  filter(rank <= 20) %>% 
  select(sml_cd, yad_reserved, count, rank)

rule_train <- train_log %>% 
  #head(1000) %>% 
  distinct(session_id, yad_no) %>% 
  left_join(yado, by = "yad_no") %>% 
  select(session_id, yad_no, sml_cd) %>% 
  group_by(session_id) %>% 
  mutate(yado_count = n_distinct(yad_no)) %>% 
  ungroup() %>% 
  left_join(reserved_sml, by = "sml_cd") %>% 
  group_by(session_id, yad_reserved, sml_cd) %>% 
...
rule_train %>% rename(yad_no = yad_reserved) %>% select(session_id, yad_no, rank) %>% write_csv("data/output/train_rule_4.csv")
rule_test %>% rename(yad_no = yad_reserved) %>% select(session_id, yad_no, rank) %>% write_csv("data/output/test_rule_4.csv")