...
#Rule6: session内に登場しているken_cdで予約数の多い宿
...
reserved_ken <- yado_merged %>% 
  arrange(ken_cd, desc(count_reserved_mod)) %>% 
  rename(yad_reserved = yad_no, count = count_reserved_mod) %>% 
  ungroup() %>% 
  group_by(ken_cd) %>% 
  mutate(rank = row_number()) %>% 
  filter(rank <= 20) %>% 
  select(ken_cd, yad_reserved, count, rank)

rule_train <- train_log %>% 
  #head(1000) %>% 
  distinct(session_id, yad_no) %>% 
  left_join(yado, by = "yad_no") %>% 
  select(session_id, yad_no, ken_cd) %>% 
  group_by(session_id) %>% 
  mutate(yado_count = n_distinct(yad_no)) %>% 
  ungroup() %>% 
  left_join(reserved_ken, by = "ken_cd") %>% 
  group_by(session_id, yad_reserved, ken_cd) %>% 
...
rule_train %>% rename(yad_no = yad_reserved) %>% select(session_id, yad_no, rank) %>% write_csv("data/output/train_rule_6.csv")
rule_test %>% rename(yad_no = yad_reserved) %>% select(session_id, yad_no, rank) %>% write_csv("data/output/test_rule_6.csv")