...
#Rule5: session内に登場しているlrg_cdで予約数の多い宿
...
reserved_lrg <- yado_merged %>% 
  arrange(lrg_cd, desc(count_reserved_mod)) %>% 
  rename(yad_reserved = yad_no, count = count_reserved_mod) %>% 
  ungroup() %>% 
  group_by(lrg_cd) %>% 
  mutate(rank = row_number()) %>% 
  filter(rank <= 20) %>% 
  select(lrg_cd, yad_reserved, count, rank)

rule_train <- train_log %>% 
  #head(1000) %>% 
  distinct(session_id, yad_no) %>% 
  left_join(yado, by = "yad_no") %>% 
  select(session_id, yad_no, lrg_cd) %>% 
  group_by(session_id) %>% 
  mutate(yado_count = n_distinct(yad_no)) %>% 
  ungroup() %>% 
  left_join(reserved_lrg, by = "lrg_cd") %>% 
  group_by(session_id, yad_reserved, lrg_cd) %>% 
...
rule_train %>% rename(yad_no = yad_reserved) %>% select(session_id, yad_no, rank) %>% write_csv("data/output/train_rule_5.csv")
rule_test %>% rename(yad_no = yad_reserved) %>% select(session_id, yad_no, rank) %>% write_csv("data/output/test_rule_5.csv")